...
setWebhookData(webhookText);
  ...
  <>
    ...
    <CustomButton onClick={openPopup}>Agregar Número</CustomButton>
          ...
          {Object.values(timeResponse)[0] && <h1>Sesiones iniciadas: {Object.values(resultadost)[0].length}</h1>}
          {Object.values(timeResponse)[0] && <h1>Sesiones atendidas: {Object.values(resultadost)[0].filter(a => a.status !== 'pending').length}</h1>}
    </div>
    </ButtonContainer>
      </Box>
