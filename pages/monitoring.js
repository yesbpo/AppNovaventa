...
setWebhookData(webhookText);
  ...
  <>
    ...
    <CustomButton onClick={openPopup}>Agregar Número</CustomButton>
          ...
          {Object.values(timeResponse)[0] && <h1>Sesiones iniciadas: {resultadost.length}</h1>}
          {Object.values(timeResponse)[0] && <h1>Sesiones atendidas: {resultadost.filter(a => a.status !== 'pending').length}</h1>}
    </div>
    </ButtonContainer>
      </Box>
