...

                        {
                            valorbuscado !== '' && < ul > {
                                datosbuscados.length > 0 ? (
                                    datosbuscados.map((contacto) => ( <
                                        ...
                                        onClick = {
                                            () => handleClick(contacto.idChat2) } > { /* Renderizar los datos del contacto */ } { nombreuser(contacto.userId) } - { contacto.idChat2 } - { contacto.status === 'pending' ? 'Pendiente' : contacto.status === 'in process' ? 'En Atención' : contacto.status === 'expiredbyassesor' ? 'Expirado por Asesor' : contacto.status === 'expiredbyclient' ? 'Expirado por cliente' : 'Finalizado' } <
                                        /li>
                                    ))
                                ) : ( <
                            ...
                            /div> {
                                resultados.map((resultado, index) => ( <
                                    ...
                                    key = { index }
                                    onClick = {
                                        () => { handlePendientesClick(resultado.asesor.id) } } > { resultado.asesor.complete_name }, estado: { resultado.asesor.session }, N° Chats: { resultado.frecuencia } <
                                    /CustomButton>
                                ))
                            }
                                                                    ...
                                                                    ul > {
                                                                        contactos1.filter(c => statuschats === '' || c.status === statuschats).map((contacto, index) => ( <
                                                                            ...
                                                                            CustomButton onClick = {
                                                                                () => setNumeroEspecifico(contacto.idChat2) } > { contacto.idChat2 }
                                                                            Asesor: { nombreuser(contacto.userId) }
                                                                            Estado: { contacto.status } < /CustomButton>

                                                                            <
                                                                            /li>
                                                                        ))
                                                                    } {
                                                                        contactos1.length == 0 && Object.values(resultadost)[0] !== undefined && Object.values(resultadost)[0].filter(c => statuschats === '' || c.status === statuschats).map((contacto, index) => ( <
                                                                            li key = { index } >
